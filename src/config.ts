...
import { loadRcFile } from "@lhci/utils/src/lighthouserc";
...
export function getInput() {
  ...
  if (!urls && !staticDistDir) {
    // Fail and exit
    core.setFailed(
      `Need either 'urls' in action parameters or a 'static_dist_dir' in lighthouserc file`
    );
    process.exit(1);
  }
...
export function hasAssertConfig(configPath: string | null) {
...
function getList(arg: string, separator = "\n") {
...
function interpolateProcessIntoUrls(urls: string[]) {
  return urls.map((url) => {
